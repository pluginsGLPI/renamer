    ...
    switch ($_POST['action']) {
        ...
        case 'restoreALanguage':
            ...
            if(PluginRenamerInstall::checkRightAccessOnGlpiLocalesFiles()){
                ...
                //clean table
                $DB->query("DELETE FROM `glpi_plugin_renamer_renamers` where `glpi_plugin_renamer_renamers`.`lang` = '".$lang."'", "renamer");
            ...
            if(PluginRenamerInstall::checkRightAccessOnGlpiLocalesFiles()){
                ...
                if($renamer->saveFileIntoTmp($file)){
                    ...
                    Session::addMessageAfterRedirect(sprintf(
                        __('Can\'t save file  \'%1$s\' in tmp folder', 'renamer'), $file ), false, INFO);
                    return false;
                }
...
function returnWarning(){
    global $CFG_GLPI;
    return "<td colspan='6'><img src='" . $CFG_GLPI['root_doc'] . "/plugins/renamer/pics/warning.png'/> "
            .__("Please give write permission to the 'locales' folder of Glpi", "renamer")."
...
function searchOnMsgid($word , $msgid , $exact = false){
    ...
    if($exact){
        foreach($msgid as $id){
            if(strtolower($id) == strtolower($word))return true;
        }
    }else{
        foreach($msgid as $id){
            if(preg_match_all("#".$word."#i",$id) ) return true;
        }
    }
...
function searchOnMsgstr($word , $msgstr,$exact = false){
    ...
    if($exact){
        foreach($msgstr as $str){
            if(strtolower($str) == stripslashes(strtolower($word)))return true;
        }
    }else{
        foreach($msgstr as $str){
...
function existExactWord($word, $id ,$string , $file){
    ...
    if ($resultString || $resultId) return true;
